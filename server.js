...
const HEARTBEAT_MAX_MS = 10 * 60 * 1000; // 10 minutos: tolerancia para Android bloqueado/background
...
app.get("/", async (req, res) => {
    ...
    res.json({
        ...
        features: [
            ...
            "webrtc_multipunto",
            "alerta_canal_broadcast",
            "background_android_tolerance"
        ]
    });
});
...
io.on("connection", (socket) => {
    ...
    socket.on("audio_completo", (data = {}) => {
        ...
        socket.emit("audio_ack", {
            mensaje: "Servidor recibió audio de " + (data.usuario || socket.data.usuario || "Usuario"),
