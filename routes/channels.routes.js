...
router.get("/listar", async (req, res) => {
    try {
        const usuarioId = req.query.usuario_id || 0;
        ...
        const [canales] = await db.query(
            `SELECT 
                ...
                c.creado_por,
                CASE 
                    WHEN cm.usuario_id IS NOT NULL AND cm.autorizado = 1 THEN 1
                    ELSE 0
                END AS autorizado
             FROM canales c
...
router.post("/validar-acceso", async (req, res) => {
    try {
        ...
        if (!canal_id || !usuario_id) {
            return res.status(400).json({
                success: false,
                error: "Canal y usuario son obligatorios"
            });
        }
...
router.post("/validar-acceso-nombre", async (req, res) => {
    try {
        ...
        if (canales.length === 0) {
            return res.status(404).json({
                success: false,
                error: "Canal no encontrado. Verifica que el nombre sea correcto."
            });
        }
