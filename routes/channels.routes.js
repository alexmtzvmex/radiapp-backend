...
router.post("/validar-acceso-nombre", async (req, res) => {
    try {
        const { nombre_canal, password_canal, usuario_id } = req.body;

        if (!nombre_canal || !usuario_id) {
            return res.status(400).json({
                success: false,
                error: "Nombre del canal y usuario son obligatorios"
            });
        }

        const [canales] = await db.query(
            `SELECT id, nombre_canal, descripcion, privado, visible_publico, password_hash, activo, creado_por
             FROM canales
             WHERE LOWER(TRIM(nombre_canal)) = LOWER(TRIM(?))
             LIMIT 1`,
            [nombre_canal]
        );
        ...
        if (canales.length === 0) {
            return res.status(404).json({
                success: false,
                error: "Canal no encontrado. Verifica que el nombre sea exactamente correcto."
            });
        }
        ...
        if (Number(canal.activo) !== 1) {
            return res.status(403).json({ success: false, error: "Canal inactivo" });
        }
        ...
        const [existente] = await db.query(
            `SELECT id FROM canal_miembros WHERE canal_id = ? AND usuario_id = ? LIMIT 1`,
            [canal.id, usuario_id]
        );

        if (existente.length > 0) {
            await db.query(
                `UPDATE canal_miembros SET autorizado = 1, rol = 'miembro' WHERE canal_id = ? AND usuario_id = ?`,
                [canal.id, usuario_id]
            );
        } else {
            await db.query(
                `INSERT INTO canal_miembros (canal_id, usuario_id, rol, autorizado) VALUES (?, ?, 'miembro', 1)`,
                [canal.id, usuario_id]
            );
        }
        ...
        res.json({
            success: true,
            message: "Acceso autorizado. El canal quedará visible para este usuario.",
            canal: {
                id: canal.id,
                nombre_canal: canal.nombre_canal,
                descripcion: canal.descripcion,
                privado: canal.privado,
                visible_publico: canal.visible_publico,
                autorizado: 1
            }
        });
    } catch (error) {
        console.error("Error validando acceso por nombre:", error);
        res.status(500).json({ success: false, error: "Error validando acceso al canal" });
    }
});
